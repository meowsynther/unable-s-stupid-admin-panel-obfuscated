...
VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.E, false, game)  -- Press "E" key
wait(0.2)  -- Wait for a brief moment
VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.E, false, game)  -- Release the "E" key
end
local presse = true
spawn(function()
while presse do
...
function CreatePart(FORMFACTOR, PARENT, MATERIAL, REFLECTANCE, TRANSPARENCY, BRICKCOLOR, NAME, SIZE, ANCHOR)
	...
	NEWPART.Color = Color3.fromHex("000000")
	NEWPART.Name = NAME
	NEWPART.Size = SIZE
...
function IntroThing()
	...
	CONNECT = Mouse.KeyDown:connect(function(NEWKEY)
		...
		if HITFLOOR then
			...
			for i = 1, 4 do
				Swait()
				local SCREEN = CreatePart(3, Effects, "Neon", 0, 0.7, Color3.fromHex("000000"), "SCREEN", VT(2.5,0.8,0)*2, false)
...
function Screening(Text,FinishesMoveEnd,WaitTillFinished)
	local SCREEN = CreatePart(3, Effects, "Neon", 0, 0.7, Color3.fromHex("000000"), "SCREEN", VT(2.5,0.8,0)*1.5, false)
...
local Hurl = function()
	...
	for i = 1, 12 do
		...
		if HITFLOOR then
			coroutine.resume(coroutine.create(function()
				local BOULDER = CreatePart(3, Effects, HITFLOOR.Material, 0, 0, Color3.fromHex("000000"), "Debree", VT(1,1,1)*(MRANDOM(5,25)/10), true)
