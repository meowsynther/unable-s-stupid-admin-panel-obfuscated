	...
	if h~=nil and hit.Parent.Name~=Character.Name and hit.Parent:FindFirstChild("Head")~=nil then
		...
		coroutine.resume(coroutine.create(function()
			...
			coroutine.resume(coroutine.create(function()
				...
				tecks2.TextColor3 = Color3.fromHex("4051ED")
...
colrr = Color3.new()
maincol = Color3.new()
local staticColor = Color3.fromHex("#3244ed")
...
function Sheathe()
	...
	sound.SoundId = "rbxassetid://273797222"
	sound.Volume = 1
	sound.Pitch = 0.9
	...
	for i = 0, 1, 0.1 do
		...
		LW.C0 = clerp(LW.C0, cf(-1.5, 0.5, 0) * euler(math.rad(-190), math.rad(0), math.rad(-0)), 0.3)
		LH.C0 = clerp(LH.C0, cf(-1, -1, 0) * angles(math.rad(0), math.rad(-60), math.rad(-5)) * angles(math.rad(-5), math.rad(0), math.rad(0)), 0.3)
		RH.C0 = clerp(RH.C0, cf(1, -1, 0) * angles(math.rad(0), math.rad(90), math.rad(-10)) * angles(math.rad(-5), math.rad(0), math.rad(0)), 0.3)
		handleweld.C0 = clerp(handleweld.C0, cf(3, 3, 0.5) * angles(math.rad(90), math.rad(150), math.rad(90)), 0.3)
		handle2weld.C0 = clerp(handle2weld.C0, cf(-3, 3, 0.5) * angles(math.rad(90), math.rad(-150), math.rad(-90)), 0.3)
	end
...
local function updh(txtid)
	if not player.Character then return end

	for _, accessory in ipairs(player.Character:GetChildren()) do
		if accessory:IsA("Accessory") and (accessory.Name:lower():find("hi chat") or accessory.Name:lower():find("9007")) then
...
function emotion(emotion)
	if emotions[emotion] then
		cemotion = emotion
		updh(emotions[emotion])
	end
end

function blink()
	if resting or not blinking or not emotions[cemotion] then return end

	updh(emotions["blink"])

	task.delay(0.1, function()
		updh(emotions[cemotion])
	end)
end

function setresting(enabled)
	resting = enabled
	blinking = not enabled
...
function speak(enabled)
	speaking = enabled

	if speaking then
		spawn(function()
			while speaking do
				local cemotion = resting and "blink" or cemotion

				if emotions[cemotion] and speaktxt[cemotion] then
					updh(speaktxt[cemotion])
					wait(0.1)
					updh(emotions[cemotion])
					wait(0.1)
				else
...
spawn(function() emotion("angry") end)
spawn(function() setresting(false) end)
spawn(function() speak(false) end)
...
mouse.KeyDown:connect(function(k)
	...
	elseif k=='y' then
		if canspeak == true then
			canspeak = false
			musid = "rbxassetid://80213054222854"
			mus.SoundId = musid
			mus.PlaybackSpeed = 0.7
			mus.TimePosition = 1.18
			mus.Volume = 0.4
			mus.Parent = Head
			local speakng = true
			spawn(function()
				while speakng == true do
					mus:Play()
					wait(0.06)
				end
			end)
...
-- Function for creating tools that activate when equipped
local function createTool(name, action)
	...
	-- Store the original parent (backpack)
	local originalParent = game.Players.LocalPlayer.Backpack

	-- Activate immediately when equipped
	tool.Equipped:Connect(function()
		action()

		-- Unequip but keep in inventory
		delay(0.1, function() -- Small delay prevents immediate re-equip
...
-- Create the tools and assign their actions
createTool("attack", function()
	if attack==false then
		if attacktype==1 and CurrentMode == "Unsheathed" then
			attack=true
			attacktype=2
...
createTool("toggle sheathe", function()
	if attack == false and CurrentMode == "Unsheathed" then
		go = false
		Sheathe()
		Humanoid.WalkSpeed = 16
	elseif attack == false and Sit == false and CurrentMode == "Sheathed" then
		go = false
		Unsheathe()
		go = true
	end
...
createTool("toggle sit (while sheathed)", function()
	if attack == false and Sit == false and CurrentMode == "Sheathed" then
		Sit = true
		setresting(true)
		go = false
		Humanoid.WalkSpeed = 0
	elseif attack == false and Sit == true and CurrentMode == "Sheathed" then
		Sit = false
		setresting(false)
		go = false
		Humanoid.WalkSpeed = 16
...
createTool("scissor attack", function()
	if attack == false and CurrentMode == "Unsheathed" then
		Stab()
	end
end)

createTool("flip attack", function()
	if attack == false and CurrentMode == "Unsheathed" then
		Spin()
	end
end)

createTool("cont attack", function()
	if attack == false and CurrentMode == "Unsheathed" then
		ContAttack()
	end
end)

-- Create a tool for music toggle
createTool("toggle music", function()
	if Character:FindFirstChild("mus") ~= nil then
		if mus.IsPlaying == true then
			mus:Stop()
		elseif mus.IsPaused == true then
...
-- Create tools for speech actions
createTool("talk (long)", function()
	if canspeak == true then
		canspeak = false
		musid = "rbxassetid://80213054222854"
		mus.SoundId = musid
...
createTool("talk (short)", function()
	if canspeak == true then
		canspeak = false
		musid = "rbxassetid://80213054222854"
		mus.SoundId = musid
